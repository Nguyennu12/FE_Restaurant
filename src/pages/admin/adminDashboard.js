    ...
    const fetchUsers = async () => {
      try {
        ...
        const response = await axios.get(
          "https://berestaurantmanagementabc-ckg8cbawb6ducpbk.japanwest-01.azurewebsites.net/api/users/search?keyword=@",
          {
  ...
  const handleRoleChange = async (user, selectedRole) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://berestaurantmanagementabc-ckg8cbawb6ducpbk.japanwest-01.azurewebsites.net/api/admin/change-role?userId=${userId}&newRoleId=${getRoleId(
          selectedRole
        )}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
