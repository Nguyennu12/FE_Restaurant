...
const FeedbackManagement = () => {
  ...
  const fetchFeedbacks = async () => {
    try {
      ...
      const response = await axios.get(
        "https://berestaurantmanagementabc-ckg8cbawb6ducpbk.japanwest-01.azurewebsites.net/api/feedback/get-all-feedback",
        {
          headers: {
  ...
  const handleUpdate = async (feedbackId, orderId) => {
    try {
      ...
      await axios.put(
        `https://berestaurantmanagementabc-ckg8cbawb6ducpbk.japanwest-01.azurewebsites.net/api/feedback/update-feedback-according-to-orderId/${feedbackId}`,
        updatedFeedback,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (feedbackId) => {
    if (window.confirm("Are you sure you want to delete this feedback?")) {
      try {
        const token = localStorage.getItem("authToken");
        await axios.delete(
          `https://berestaurantmanagementabc-ckg8cbawb6ducpbk.japanwest-01.azurewebsites.net/api/feedback/delete-feedback/${feedbackId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
